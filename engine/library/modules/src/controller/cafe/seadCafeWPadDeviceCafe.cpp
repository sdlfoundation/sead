...
CafeWPadDevice::CafeWPadDevice(ControllerMgr *mgr, Heap *heap)
    : ControlDevice(mgr), mFreezeCounter(0) {
...
CafeWPadDevice::~CafeWPadDevice() { KPADShutdown(); }
...
void CafeWPadDevice::calc() {
  for (u32 i = 0; i < WPAD_MAX_CONTROLLERS; i++) {
    if (mFreezeCounter > 0) {
      KPADStatus status;
      s32 err;
      (void)KPADReadEx(i, &status, 1, &err);
    } else {
      mKPadInfos[i].last_read_length =
          KPADReadEx(i, mKPadInfos[i].status, KPAD_MAX_READ_BUFS,
