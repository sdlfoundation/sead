...
PrimitiveRendererCafe::PrimitiveRendererCafe(Heap *heap)
    : mCameraMtx(), mProjectionMtx() {}

void PrimitiveRendererCafe::prepareFromBinaryImpl(Heap *heap,
                                                  const void *bin_data,
                                                  u32 bin_size) {
  ...
  GX2Invalidate(GX2_INVALIDATE_CPU_SHADER, mVertexShader->shaderPtr,
                mVertexShader->shaderSize);
  ...
  GX2Invalidate(GX2_INVALIDATE_CPU_SHADER, mPixelShader->shaderPtr,
                mPixelShader->shaderSize);
  ...
  mAttrTexCoord0Location =
      GX2GetVertexAttribVarLocation(mVertexShader, "TexCoord0");
  mAttrColorRateLocation =
      GX2GetVertexAttribVarLocation(mVertexShader, "ColorRate");

  GX2InitAttribStream(&mAttributes[0], mAttrVertexLocation, 0, 0,
                      GX2_ATTRIB_FORMAT_32_32_32_FLOAT);
  GX2InitAttribStream(&mAttributes[1], mAttrTexCoord0Location, 0,
                      sizeof(f32) * 3, GX2_ATTRIB_FORMAT_32_32_FLOAT);
  GX2InitAttribStream(&mAttributes[2], mAttrColorRateLocation, 0,
                      sizeof(f32) * 5, GX2_ATTRIB_FORMAT_32_32_32_32_FLOAT);

  mFetchShaderBufPtr =
      heap->alloc(GX2CalcFetchShaderSize(3), GX2_SHADER_ALIGNMENT);

  GX2InitFetchShader(&mFetchShader, mFetchShaderBufPtr, 3, mAttributes);
  GX2Invalidate(GX2_INVALIDATE_CPU_SHADER, mFetchShaderBufPtr,
                GX2CalcFetchShaderSize(3));

  {
    // Quad
    mQuadVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc(4 * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mQuadIndexBuf = static_cast<u16 *>(
        heap->alloc(6 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    ...
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mQuadIndexBuf,
                  6 * sizeof(u16));
  }

  {
    ...
    mBoxIndexBuf = static_cast<u16 *>(
        heap->alloc(sizeof(idx), GX2_INDEX_BUFFER_ALIGNMENT));
    MemUtil::copy(mBoxIndexBuf, idx, sizeof(idx));

    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mBoxIndexBuf,
                  4 * sizeof(u16));
  }

  {
    ...
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mLineVertexBuf,
                  4 * sizeof(PrimitiveRendererUtil::Vertex));
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mLineIndexBuf,
                  6 * sizeof(u16));
  }

  {
    ...
    mCubeVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc(8 * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mCubeIndexBuf = static_cast<u16 *>(
        heap->alloc(36 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    ...
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mCubeIndexBuf,
                  36 * sizeof(u16));
  }

  {
    ...
    mWireCubeVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc(8 * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mWireCubeIndexBuf = static_cast<u16 *>(
        heap->alloc(17 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    PrimitiveRendererUtil::setWireCubeVertex(mWireCubeVertexBuf,
                                             mWireCubeIndexBuf);

    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mWireCubeVertexBuf,
                  8 * sizeof(PrimitiveRendererUtil::Vertex));
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mWireCubeIndexBuf,
                  17 * sizeof(u16));
  }

  {
    // SphereS
    mSphereSVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc((4 * 8 + 2) * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mSphereSIndexBuf = static_cast<u16 *>(
        heap->alloc((4 * 8 * 6) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    PrimitiveRendererUtil::setSphereVertex(mSphereSVertexBuf, mSphereSIndexBuf,
                                           8, 4);
  ...
  {
    // SphereL
    mSphereLVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc((8 * 16 + 2) * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mSphereLIndexBuf = static_cast<u16 *>(
        heap->alloc((8 * 16 * 6) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    PrimitiveRendererUtil::setSphereVertex(mSphereLVertexBuf, mSphereLIndexBuf,
                                           16, 8);
  ...
  {
    // DiskS
    mDiskSVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc((16 + 1) * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mDiskSIndexBuf = static_cast<u16 *>(
        heap->alloc((8 * 6) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    ...
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mDiskSIndexBuf,
                  (8 * 6) * sizeof(u16));
  }

  {
    // DiskL
    mDiskLVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc((32 + 1) * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mDiskLIndexBuf = static_cast<u16 *>(
        heap->alloc((16 * 6) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    ...
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mDiskLIndexBuf,
                  (16 * 6) * sizeof(u16));
  }

  {
    // CircleS
    mCircleSIndexBuf = static_cast<u16 *>(
        heap->alloc(16 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    for (s32 i = 0; i < 16; i++)
      mCircleSIndexBuf[i] = i;
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mCircleSIndexBuf,
                  16 * sizeof(u16));
  }

  {
    // CircleL
    mCircleLIndexBuf = static_cast<u16 *>(
        heap->alloc(32 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    for (s32 i = 0; i < 32; i++)
      mCircleLIndexBuf[i] = i;
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mCircleLIndexBuf,
                  32 * sizeof(u16));
  }

  {
    // CylinderS
    mCylinderSVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc((16 * 2 + 2) * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mCylinderSIndexBuf = static_cast<u16 *>(
        heap->alloc((16 * 12) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    PrimitiveRendererUtil::setCylinderVertex(mCylinderSVertexBuf,
                                             mCylinderSIndexBuf, 16);
  ...
  {
    // CylinderL
    mCylinderLVertexBuf = static_cast<PrimitiveRendererUtil::Vertex *>(
        heap->alloc((32 * 2 + 2) * sizeof(PrimitiveRendererUtil::Vertex),
                    GX2_VERTEX_BUFFER_ALIGNMENT));
    mCylinderLIndexBuf = static_cast<u16 *>(
        heap->alloc((32 * 12) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
    PrimitiveRendererUtil::setCylinderVertex(mCylinderLVertexBuf,
                                             mCylinderLIndexBuf, 32);
  ...
  GX2InitSampler(&mDrawQuadSampler, GX2_TEX_CLAMP_CLAMP,
...
void PrimitiveRendererCafe::drawQuadImpl(const Matrix34f &model_mtx,
...
void PrimitiveRendererCafe::drawQuadImpl(
  ...
  drawTriangles_(model_mtx, colorL, colorR, mQuadVertexBuf, 4, mQuadIndexBuf, 6,
                 texure_cafe_gx2->getGX2Texture());
}

void PrimitiveRendererCafe::drawBoxImpl(const Matrix34f &model_mtx,
                                        const Color4f &colorL,
                                        const Color4f &colorR) {
  drawLines_(model_mtx, colorL, colorR, mQuadVertexBuf, 4, mBoxIndexBuf, 4);
}

void PrimitiveRendererCafe::drawCubeImpl(const Matrix34f &model_mtx,
                                         const Color4f &c0, const Color4f &c1) {
  drawTriangles_(model_mtx, c0, c1, mCubeVertexBuf, 8, mCubeIndexBuf, 36,
                 nullptr);
}

void PrimitiveRendererCafe::drawWireCubeImpl(const Matrix34f &model_mtx,
                                             const Color4f &c0,
                                             const Color4f &c1) {
  drawLines_(model_mtx, c0, c1, mWireCubeVertexBuf, 8, mWireCubeIndexBuf, 17);
}

void PrimitiveRendererCafe::drawLineImpl(const Matrix34f &model_mtx,
                                         const Color4f &c0, const Color4f &c1) {
  drawLines_(model_mtx, c0, c1, mLineVertexBuf, 2, mLineIndexBuf, 2);
}

void PrimitiveRendererCafe::drawSphere4x8Impl(const Matrix34f &model_mtx,
                                              const Color4f &north,
                                              const Color4f &south) {
  drawTriangles_(model_mtx, north, south, mSphereSVertexBuf, 34,
                 mSphereSIndexBuf, 192, nullptr);
}

void PrimitiveRendererCafe::drawSphere8x16Impl(const Matrix34f &model_mtx,
                                               const Color4f &north,
                                               const Color4f &south) {
  drawTriangles_(model_mtx, north, south, mSphereLVertexBuf, 130,
                 mSphereLIndexBuf, 768, nullptr);
}

void PrimitiveRendererCafe::drawDisk16Impl(const Matrix34f &model_mtx,
                                           const Color4f &center,
                                           const Color4f &edge) {
  drawTriangles_(model_mtx, center, edge, mDiskSVertexBuf, 17, mDiskSIndexBuf,
                 48, nullptr);
}

void PrimitiveRendererCafe::drawDisk32Impl(const Matrix34f &model_mtx,
                                           const Color4f &center,
                                           const Color4f &edge) {
  drawTriangles_(model_mtx, center, edge, mDiskLVertexBuf, 33, mDiskLIndexBuf,
                 96, nullptr);
}

void PrimitiveRendererCafe::drawCircle16Impl(const Matrix34f &model_mtx,
                                             const Color4f &edge) {
  drawLines_(model_mtx, edge, edge, mDiskSVertexBuf, 17, mCircleSIndexBuf, 16);
}

void PrimitiveRendererCafe::drawCircle32Impl(const Matrix34f &model_mtx,
                                             const Color4f &edge) {
  drawLines_(model_mtx, edge, edge, mDiskLVertexBuf, 33, mCircleLIndexBuf, 32);
}

void PrimitiveRendererCafe::drawCylinder16Impl(const Matrix34f &model_mtx,
                                               const Color4f &top,
                                               const Color4f &btm) {
  drawTriangles_(model_mtx, top, btm, mCylinderSVertexBuf, 34,
                 mCylinderSIndexBuf, 192, nullptr);
}

void PrimitiveRendererCafe::drawCylinder32Impl(const Matrix34f &model_mtx,
                                               const Color4f &top,
                                               const Color4f &btm) {
  drawTriangles_(model_mtx, top, btm, mCylinderLVertexBuf, 66,
                 mCylinderLIndexBuf, 384, nullptr);
}

void PrimitiveRendererCafe::drawTriangles_(const Matrix34f &model_mtx,
                                           const Color4f &c0, const Color4f &c1,
...
void PrimitiveRendererCafe::drawLines_(const Matrix34f &model_mtx,
                                       const Color4f &c0, const Color4f &c1,
                                       PrimitiveRendererUtil::Vertex *vtx,
                                       u32 vtx_num, u16 *idx, u32 idx_num) {
  GX2SetVertexUniformReg(mParamUserOffset, 12, &model_mtx);
