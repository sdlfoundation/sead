...
public:
  struct KPadInfo {
    KPadInfo()
        : last_read_length(0), last_read_error(KPAD_READ_ERR_NO_CONTROLLER) {
  ...
  static_assert(sizeof(KPadInfo) == 0xF08,
...
public:
  ...
  const KPadInfo &getKPadInfo(s32 channel) const { return mKPadInfos[channel]; }

  void setFreezeCounter(s32 value) { mFreezeCounter = value; }
...
static_assert(sizeof(CafeWPadDevice) == 0x4D40,
